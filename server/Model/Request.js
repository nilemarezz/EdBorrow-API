...
class ItemRequest {
  ...
  async advisorApprove (query) {
    if (query.approver === 'advisor' && query.status === 'TRUE') {
      await pool.query (`UPDATE BorrowRequest SET requestApprove = TRUE WHERE requestId = ${query.requestId};`);
  ...
  async departmentApprove (query) {
    if (query.approver === 'department' && query.status === 'TRUE') {
      await pool.query (`
      UPDATE RequestItem ri join Items i on ri.itemId = i.itemId 
      SET ri.itemApprove = TRUE , ri.itemBorrowingStatusId = 1 , i.itemAvailability = FALSE
